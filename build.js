...
function createZip(sourceDir, outputFile) {
    ...
    try {
        execSync(`cd "${sourceDir}" && zip -r "${outputFile}" . -x ".*" -x "*.md"`, {
            stdio: 'ignore'
        });
        return;
    } catch (e) {
...
async function main() {
    ...
    const tempDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'autolister-build-'));
    ...
    try {
        ...
        if (fs.existsSync(outputFile)) {
            ...
            // List files in zip
            try {
                const output = execSync(`unzip -l "${outputFile}"`, { encoding: 'utf8' });
